...
public class MediaMuxerWrapper {
	...
	public MediaMuxerWrapper(String ext, String folder, String name) throws IOException {
		if (TextUtils.isEmpty(ext)) ext = ".mp4";
		try {
			mOutputPath = getVideoFile(folder, name, ext).toString();
		} catch (final NullPointerException e) {
			throw new RuntimeException("This app has no permission of writing external storage");
		}
		mMediaMuxer = new MediaMuxer(mOutputPath, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
...
//**********************************************************************
	...
	public static final File getVideoFile(final String folder, final String name, final String ext) {
		final File dir = new File("/sdcard/CloudVending/videoFile/" + folder);
		Log.d(TAG, "path=" + dir.toString());
		dir.mkdirs();
		if (dir.canWrite()) {
			return new File(dir, getDateTimeString() + "_" + name + ext);
		}
		return null;
	}
