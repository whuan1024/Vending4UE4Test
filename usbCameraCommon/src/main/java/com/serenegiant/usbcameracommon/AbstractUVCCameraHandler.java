...
public abstract class AbstractUVCCameraHandler extends Handler {
	...
	private static HashMap<Integer, Boolean> mDeviceCaptureMap = new HashMap<>();
	private static OnCaptureListener mCaptureListener;

	public interface OnCaptureListener {
		void onCaptureResult(int deviceId, byte[] data, int width, int height);
	}
	...
	protected void captureStill(final int deviceId, final OnCaptureListener listener) {
		checkReleased();
		mCaptureListener = listener;
		mDeviceCaptureMap.put(deviceId, true);
	}

	public void startRecording(String eventId, int deviceId) {
		checkReleased();
		Message message = Message.obtain(this, MSG_CAPTURE_START, deviceId, 0, eventId);
		sendMessage(message);
	}
	...
	public void handleMessage(final Message msg) {
		...
		case MSG_CAPTURE_START:
			thread.handleStartRecording((String) msg.obj, msg.arg1);
			break;
	...
	static final class CameraThread extends Thread {
		...
		private final CameraViewInterface mWeakCameraView;
		...
		public void handleOpen(final USBMonitor.UsbControlBlock ctrlBlock) {
			...
			try {
				...
				synchronized (mSync) {
					mUVCCamera = camera;
					mDeviceId = camera.getDevice().getDeviceId();
				}
				callOnOpen();
		...
		public void handleCaptureStill(final String path) {
			...
			try {
				final Bitmap bitmap = mWeakCameraView.captureStillImage();
		...
		public void handleStartRecording(String eventId, int deviceId) {
			if (DEBUG) Log.v(TAG_THREAD, "handleStartRecording:--mUVCCamera:"+mUVCCamera+"////mMuxer:"+mMuxer+"////eventId:"+eventId+"////deviceId:"+deviceId);
			try {
				if ((mUVCCamera == null) || (mMuxer != null)) return;
				final MediaMuxerWrapper muxer = new MediaMuxerWrapper(".mp4", eventId, String.valueOf(deviceId));	// if you record audio only, ".m4a" is also OK.
...
//				}
		...
		public void handleStopRecording() {
			...
			try {
				mWeakCameraView.setVideoEncoder(null);
			} catch (final Exception e) {
				// ignore
			}
			if (muxer != null) {
		...
		private boolean needCapture(int key) {
			Boolean value = mDeviceCaptureMap.get(key);
					...
					new Thread(new Runnable() {
						...
						public void run() {
							if (mCaptureListener != null) {
								mCaptureListener.onCaptureResult(mDeviceId, yuv, mWidth, mHeight);
							}
						}
					}).start();
				}
		...
		private final MediaEncoder.MediaEncoderListener mMediaEncoderListener = new MediaEncoder.MediaEncoderListener() {
			...
			public void onPrepared(final MediaEncoder encoder) {
				...
				try {
					//Log.w(TAG, "onPrepare--mWeakCameraView.get():"+mWeakCameraView.get());
					Log.w(TAG, "onPrepare--mWeakCameraView:"+mWeakCameraView);
					mWeakCameraView.setVideoEncoder((MediaVideoEncoder)encoder);
				} catch (final Exception e) {
					Log.e(TAG, "onPrepared:", e);
				}
				if (encoder instanceof MediaSurfaceEncoder)
				try {
					mWeakCameraView.setVideoEncoder((MediaSurfaceEncoder)encoder);
